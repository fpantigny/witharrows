...
%% 
\def\myfileversion{2.9x}
\def\myfiledate{2025/11/03}
...
\cs_new_protected:Npn \__witharrows_error_or_warning:n
  {
    \bool_if:NTF \c__witharrows_messages_for_Overleaf_bool
      { \__witharrows_warning:n }
      { \__witharrows_error:n }
  }
\bool_const:Nn \c__witharrows_messages_for_Overleaf_bool
  {
...
\cs_new_protected:Npn \__witharrows_save:N #1
  {
    ...
    \use:c { \l_tmpa_str _if_exist:cF }
      { g _\seq_use:Nn \l_tmpa_seq { _ } }
      {
        \use:c { \l_tmpa_str _new:c }
          { g _\seq_use:Nn \l_tmpa_seq { _ } }
      }
    \use:c { \l_tmpa_str _gset_eq:cN }
      { g _\seq_use:Nn \l_tmpa_seq { _ } } #1
  }
...
\tl_new:N \l__witharrows_replace_left_brace_by_tl
...
\keys_define:nn { WithArrows / Global }
  {
    ...
    more-columns .code:n =
      \__witharrows_msg_redirect_name:nn { Too~many~columns~in~WithArrows } { none } ,
...
\clist_new:N \g__witharrows_ext_delimiters_clist
\clist_gset:Nn \g__witharrows_ext_delimiters_clist
  {
    ...
    replace-left-brace-by .code:n =
      {
        ...
        \clist_if_in:NoTF
          \g__witharrows_ext_delimiters_clist
          \l_tmpa_tl
...
\keys_define:nn { WithArrows / Env }
  {
    ...
    format .code:n =
      \tl_if_empty:nTF { #1 }
        ...
        {
          \regex_if_match:nnTF { \A[rclRCL]*\Z } { #1 }
...
\cs_new_protected:Npn \__witharrows_Arrow_ii [ #1 ] #2 [ #3 ]
  {
    ...
    \prop_clear_new_linked:N \l_tmpa_linked_prop
    \prop_put:NnV \l_tmpa_linked_prop { initial } \g__witharrows_line_int
    \int_set:Nn \l_tmpa_int { \g__witharrows_line_int + \l__witharrows_jump_int }
    \prop_put:NnV \l_tmpa_linked_prop { final } \l_tmpa_int
    \prop_put:Nno \l_tmpa_linked_prop { status } { \l__witharrows_status_arrow_str }
    \prop_put:Nnn \l_tmpa_linked_prop { options } { #1 , #3 }
    \prop_put:Nnn \l_tmpa_linked_prop { label } { #2 }
    \prop_put:Nne \l_tmpa_linked_prop { input-line } { \msg_line_number: }
    \prop_put:Nnn \l_tmpa_linked_prop { width } { 0 pt }
...
\cs_new_protected:Npn \__witharrows_pre_halign:n #1
  {
    ...
    \str_set:Ne \l__witharrows_prefix_str { \seq_use:Nn \l_tmpa_seq { - } }
...
\cs_new_protected:Npn \WithArrows_i [ #1 ]
  {
    ...
    \int_compare:nNnT { \l__witharrows_pos_env_int } = { 1 }
      { $ } % $
    ...
    \__witharrows_construct_halign:
    &&
    \__witharrows_error:n { Too~many~columns~in~WithArrows }
    $ % $
    \bool_if:NT \l__witharrows_displaystyle_bool { \displaystyle }
    { ## }
    $ % $
...
\cs_new_protected:Npn \DispWithArrows_ii < #1 > [ #2 ]
  {
    ...
    \tabskip = \c_zero_skip
    &&
    \__witharrows_fatal:n { Too~many~columns~in~DispWithArrows }
            ...
            \hbox_to_wd:nn \g__witharrows_alignment_dim
              {
                ...
                \dim_set:Nn \l_tmpa_dim
                  { \box_ht_plus_dp:N \l__witharrows_halign_box }
                \group_begin:
                \dim_zero:N \nulldelimiterspace
                $ % $
...
\cs_new_protected:Npn \__witharrows_scan_arrows_i:
  {
    ...
    \bool_lazy_and:nnT
      ...
      {
        \int_compare_p:nNn
          { \l__witharrows_first_arrow_of_group_int } > { \c_zero_int }
      }
      {
        \__witharrows_draw_arrows:nn
          { \l__witharrows_first_arrow_of_group_int }
          { \g__witharrows_arrow_int }
      }
  }
...
\cs_new_protected:Npn \__witharrows_draw_arrows:nn #1 #2
  {
    ...
    \int_set_eq:NN \l__witharrows_arrow_int \l__witharrows_first_arrow_int
...
\cs_new_protected:Npn \__witharrows_draw_arrow:
  {
    ...
    \tl_set:Ne \l__witharrows_initial_tl
      {
        \int_use:N \l__witharrows_initial_int -
        \bool_if:NTF \l__witharrows_initial_r_bool { r } { l }
      }
    \tl_set:Ne \l__witharrows_final_tl
      {
        \int_use:N \l__witharrows_final_int -
        \bool_if:NTF \l__witharrows_final_r_bool { r } { l }
      }
    \prop_get:cnN
      { g__witharrows_arrow _ \l__witharrows_prefix_str _ \int_use:N \l__witharrows_arrow_int _ prop }
      { label }
...
\tl_const:Nn \c__witharrows_tikz_code_wrap_lines_tl
  {
    ...
    \tl_if_empty:NF \g_tmpa_tl
      {
        \dim_set_eq:NN \l_tmpb_dim \g_tmpa_tl
...
\cs_new_protected:Npn \__witharrows_draw_o_arrows_of_the_group:
  {
    \bool_lazy_all:nTF
      {
        \l__witharrows_wrap_lines_bool
        \l__witharrows_in_DispWithArrows_bool
        { ! \seq_if_empty_p:N \l__witharrows_o_arrows_seq }
      }
      { \__witharrows_error:n { o~arrow~with~wrap-lines } }
      { \__witharrows_draw_o_arrows_of_the_group_i: }
  }
\cs_new_protected:Npn \__witharrows_draw_o_arrows_of_the_group_i:
  {
...
\cs_new_protected:Npn \__witharrows_err_newline_at_the_end:
  {
    \__witharrows_error:n { newline~at~the~end }
    \cs_gset:Npn \__witharrows_err_newline_at_the_end: { }
  }
\cs_new_protected:Npn \__witharrows_err_amsmath_not_loaded:
  {
    \__witharrows_error:n { amsmath~not~loaded }
    \cs_gset:Npn \__witharrows_err_amsmath_not_loaded: { \__witharrows_info:n { amsmath~not~loaded } }
  }
\bool_if:NTF \c__witharrows_messages_for_Overleaf_bool
...
\__witharrows_msg_new:nn { DispWithArrows~with~tagging }
  {
    You~can't~use~\{DispWithArrows\}~since~tagging~is~active.\\
    That~error~is~fatal.
  }
...
\__witharrows_msg_new:nnn { Too~many~columns~in~WithArrows }
  {
    Too~many~columns.\\
  ...
  { \__witharrows_potential_body_ii: }
\__witharrows_msg_new:nnn { Too~many~columns~in~DispWithArrows }
  {
    Too~many~columns.\\
    Your~environment~\{\l__witharrows_type_env_str\}~has~\int_use:N
    \l__witharrows_nb_cols_int\ columns~and~you~try~to~use~one~more.~
